...
def move_obstacles(obstacles):
    ...
    for obstacle in obstacles:
        obstacle.centerx -= 2.5         # control obstacle speed
    #f 50 < score < 100:
...
while True:
    ...
    for event in pygame.event.get():                 # event loop : looks for events 
        ...
        if event.type == pygame.KEYDOWN:                        # takes user input to move heli
            if event.key == pygame.K_SPACE and game_active:     # and start_game == True:
                heli_movement = 0
                heli_movement -= 4                                          # heli move up, HELI SPEED  
                flap_sound.play()                                           # heli sound
    ...
    if game_active and start_game:
        ...
        start_surface = game_font.render(str("Press Space to Start Game"),True,(255,255,255))
